...
def main():
    ...
    with LeaseKeepAlive(lease_client, must_acquire=True, return_at_exit=True):
        ...
        try:
            return relative_move(options, options.dx, options.dy, math.radians(options.dyaw), options.frame,
...
def return_to_base(options, dx, dy, dyaw, frame_name, robot_command_client, robot_state_client, stairs=False):
...
def relative_move(options, dx, dy, dyaw, frame_name, robot_command_client, robot_state_client, stairs=False):
    transforms = robot_state_client.get_robot_state().kinematic_state.transforms_snapshot
    ...
    while True:
        ...
        if mobility_feedback.status != RobotCommandFeedbackStatus.STATUS_PROCESSING:
            print("Failed to reach the goal")
            relative_move(options, 0, 0, math.radians(options.dyaw), options.frame,
                                 robot_command_client, robot_state_client, stairs=options.stairs)
            return False
        traj_feedback = mobility_feedback.se2_trajectory_feedback
        if (traj_feedback.status == traj_feedback.STATUS_AT_GOAL and
                traj_feedback.body_movement_status == traj_feedback.BODY_STATUS_SETTLED):
            print("Arrived at the goal.")
            return_to_base(options, -dx, -dy, math.radians(options.dyaw), options.frame,
                                 robot_command_client, robot_state_client, stairs=options.stairs)
            return True
        time.sleep(1)
